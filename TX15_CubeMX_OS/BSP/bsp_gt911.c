...
#define USE_JingLei		0
#define USE_Fire		0
#define USE_HUaZuan		1

#if USE_HUaZuan	
const uint8_t CTP_CFG_GT911[] = {
	0x61,0xE0,0x01,0x40,0x01,0x05,0xBD,0x00,0x02,0x08,
	0x1E,0x08,0x50,0x3C,0x0F,0x05,0x00,0x00,0x00,0x00,
	0x50,0x00,0x00,0x18,0x1A,0x1E,0x14,0x87,0x27,0x0A,
	0x4B,0x4D,0xD3,0x07,0x00,0x00,0x00,0x02,0x32,0x1D,
	...
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x99,0x01
};		
#endif
#if USE_JingLei
const uint8_t CTP_CFG_GT911[] = {
	
    0x42, 0xE0, 0x01, 0x40, 0x01, 0x0A, 0x39, 0x00, 0x02, 0x0F,
    0x28, 0x0F, 0x5A, 0x3C, 0x03, 0x05, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x17, 0x19, 0x1D, 0x14, 0x87, 0x27, 0x09,
    0x37, 0x39, 0xD9, 0x0B, 0x00, 0x00, 0x00, 0x83, 0x02, 0x1C,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x24, 0x0C,
    0x0A, 0x08, 0x06, 0x04, 0x02, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x2E, 0x01
};
#endif
...
void touch_gt911_init(void)
{
    ...
    if (touchIC == GT911)
    {
        GTP_INFO("touchIC = GT911");
        cfg_info = CTP_CFG_GT911;      //配置数组              // 指向寄存器配置
    ...
    /*根据LCD的扫描方向设置分辨率*/
//   		config[GTP_ADDR_LENGTH+1] = 320 & 0xFF;
//   		config[GTP_ADDR_LENGTH+2] = 320 >> 8;
//   		config[GTP_ADDR_LENGTH+3] = 480 & 0xFF;
//   		config[GTP_ADDR_LENGTH+4] = 480 >> 8;

    config[GTP_ADDR_LENGTH + 1] = 480 & 0xFF;
    config[GTP_ADDR_LENGTH + 2] = 480 >> 8;
    config[GTP_ADDR_LENGTH + 3] = 320 & 0xFF;
    config[GTP_ADDR_LENGTH + 4] = 320 >> 8;
 ...
 *        参考 Goodix_TS_Work_Func 修改而来， 只读取单个触摸点坐标
...
uint16_t Touch_X = 0;
uint16_t Touch_Y = 0;
uint32_t touch_scan_tick = 0;
void touch_Loop(void)
{
    if ((HAL_GetTick() - touch_scan_tick) > 18)
    {
        GTP_Execu(&Touch_X, &Touch_Y);

        touch_scan_tick = HAL_GetTick();
    }
}
