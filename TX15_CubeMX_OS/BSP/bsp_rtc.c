...
void RTC_TimeAndDate_Show(void)
{
	...
	if((HAL_GetTick() - get_rtc_tick) > 999)
	{
		// 获取日历
		...
		if(Rtctmp != RTC_TimeStructure.Seconds)
		{
			...
			// 打印日期
//			debug_tx5("The Date :  Y:20%0.2d - M:%0.2d - D:%0.2d - W:%0.2d\r\n", 
...
//			RTC_DateStructure.WeekDay);
			...
			//液晶显示日期
			//先把要显示的数据用sdebug_tx5函数转换为字符串，然后才能用液晶显示函数显示
			sprintf(LCDDate,"The Date :  Y:20%0.2d - M:%0.2d - D:%0.2d - W:%0.2d", 
			RTC_DateStructure.Year,
			RTC_DateStructure.Month, 
			RTC_DateStructure.Date,
			RTC_DateStructure.WeekDay);

			DisRtc.Year = RTC_DateStructure.Year;
			DisRtc.Month = RTC_DateStructure.Month;
			DisRtc.Date = RTC_DateStructure.Date;			
//			LCD_SetColors(LCD_COLOR_RED,LCD_COLOR_BLACK);
//			LCD_DisplayStringLine_EN_CH(8,(uint8_t *)LCDTemp); 
			
			// 打印时间
//			debug_tx5("The Time :  %0.2d:%0.2d:%0.2d ", 
//			RTC_TimeStructure.Hours, 
//			RTC_TimeStructure.Minutes, 
//			RTC_TimeStructure.Seconds);
			
			//液晶显示时间
			sprintf(LCDTime,"The Time :  %0.2d:%0.2d:%0.2d", 
			RTC_TimeStructure.Hours, 
			RTC_TimeStructure.Minutes, 
			RTC_TimeStructure.Seconds);

			DisRtc.Hours = RTC_TimeStructure.Hours;
			DisRtc.Minutes = RTC_TimeStructure.Minutes;
			DisRtc.Seconds = RTC_TimeStructure.Seconds;
//			LCD_DisplayStringLine_EN_CH(10,(uint8_t *)LCDTemp);
				
		  (void)RTC->DR;
		}
		Rtctmp = RTC_TimeStructure.Seconds;
		get_rtc_tick = HAL_GetTick();
	}	
}
