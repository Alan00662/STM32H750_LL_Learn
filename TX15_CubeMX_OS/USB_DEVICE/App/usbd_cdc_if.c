...
*	返 回 值:	无
*	函数功能:	USB格式化输出
*	说    明:	使用类似printf，通过USB CDC 虚拟串口，将数据格式化输出，PC端可用串口助手接收，	
...
void USB_printf(const char *format, ...)
{
	...
	USB_TimeOut = 3;	// 超时等待时间，这里取3ms，实测单次发送2K的数据将近2ms，用户可以根据实际需求进行调整

	if( hUsbDeviceFS.dev_state == USBD_STATE_CONFIGURED )		// 判断USB是否处于连接状态
	{
		USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*)hUsbDeviceFS.pClassData;	// 获取相应的CDC状态
