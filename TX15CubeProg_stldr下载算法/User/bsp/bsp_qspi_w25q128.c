...
int QSPI_WriteBuffer(uint8_t *_pBuf, uint32_t _uiWriteAddr, uint16_t _usWriteSize)
{
    ...
    /* 写序列配置 */
    sCommand.Instruction = QUAD_IN_FAST_PROG_CMD;   /* 32bit地址的4线快速写入命令 */
    ...
    /* 启动传输 */
    if (HAL_QSPI_Transmit(&QSPIHandle, _pBuf, HAL_MAX_DELAY) != HAL_OK)
    {
         return 1;
    }
...
static int QSPI_AutoPollingMemReady(QSPI_HandleTypeDef *hqspi)
{
    ...
    if (HAL_QSPI_AutoPolling(&QSPIHandle, &sCommand, &sConfig, HAL_MAX_DELAY) != HAL_OK)
    {
        return 1;
    }
